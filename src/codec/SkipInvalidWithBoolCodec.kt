...
class SkipInvalidWithBoolCodec : ICodec {
    ...
    override fun decodeGrid(binaryGrid: String): CoordinateGrid {
        ...
        while (binaryGridStringBuilder.isNotEmpty() && binaryGridStringBuilder.length >= 13) {
            ...
            val nextBuildingId = binaryGridStringBuilder.substring(1, 13).toInt(2)
            // remove so the padded 0s at the end don't count
            buildingsOrEmpty.add(idToBuildingMap.remove(nextBuildingId))
            binaryGridStringBuilder.deleteRange(0, 13)
        }
